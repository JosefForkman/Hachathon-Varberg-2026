...
export function classifyType(raw) {
  ...
  if (key === 'nationalpark') return { key, label: 'National park', cssMod: 'park' }
  if (key === 'naturreservat') return { key, label: 'Reserve', cssMod: 'reserve' }
  if (key === 'natura 2000') return { key, label: 'Natura 2000', cssMod: 'reserve' }
  if (key === 'naturvardsomrade') return { key, label: 'Conservation', cssMod: 'other' }
...
export function computeScore(area, weather) {
  ...
  if (weather) {
    ...
    if (weather.wind != null && weather.wind > 10) climate -= 0.10
    if (weather.symbol === 11 || weather.symbol === 21) climate -= 0.10
  }
  ...
  const total = Math.round(0.4 * biodiversity + 0.35 * climate + 0.25 * ecosystem)
